...
func TestSortTasks(t *testing.T) {
	...
	t.Run("sort by priority then due date", func(t *testing.T) {
		entries := []*entry.Entry{
			{Title: "Low priority, soon", Priority: "low", Due: today.Add(1 * 24 * time.Hour).Format("2006-01-02")},
			{Title: "High priority, later", Priority: "high", Due: today.Add(5 * 24 * time.Hour).Format("2006-01-02")},
			{Title: "High priority, soon", Priority: "high", Due: today.Add(2 * 24 * time.Hour).Format("2006-01-02")},
			{Title: "Critical, no due", Priority: "critical", Due: ""},
		}

		sortTasks(entries)

		// Expected order: critical > high (by due) > low
		if entries[0].Title != "Critical, no due" {
			t.Errorf("expected critical priority first, got %q", entries[0].Title)
		}
		if entries[1].Priority != "high" {
			t.Errorf("expected high priority second, got %q", entries[1].Priority)
		}
	})
	...
	t.Run("single entry", func(t *testing.T) {
		entries := []*entry.Entry{{Title: "Single", Priority: "high"}}
		sortTasks(entries)
		if len(entries) != 1 || entries[0].Title != "Single" {
			t.Errorf("single entry should remain unchanged")
		}
	})
}

// Test parseDueDate function
func TestParseDueDate(t *testing.T) {
	tests := []struct {
		name    string
		input   string
		isZero  bool
		dateStr string // expected date string
	}{
		{
			name:   "empty",
			input:  "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.isZero && !result.IsZero() {
				t.Errorf("parseDueDate(%q) should be zero time", tt.input)
			}
			if !tt.isZero {
				if result.IsZero() {
					t.Errorf("parseDueDate(%q) should not be zero time", tt.input)
				} else if result.Format("2006-01-02") != tt.dateStr {
					t.Errorf("parseDueDate(%q) = %s, want %s", tt.input, result.Format("2006-01-02"), tt.dateStr)
				}
			}
		})
	}
